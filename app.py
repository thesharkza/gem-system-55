...
# 🌟 อัปเกรด 1: Complexity Aversion Penalty (หักลบความน่าจะเป็นในเรตราคาควบลูก)
def apply_complexity_penalty(ev, line):
    rm = line - math.floor(line)
    if rm == 0.25 or rm == 0.75:
        return ev - 0.015 # หัก EV ออก 1.5% หากเป็นเรต .25 หรือ .75 เพื่อชดเชยค่าซ่อนกำไรของบ่อน
    return ev

def ev_ah(hdp,w2,w1,d,l1,l2,odds,fav):
    b=odds-1; h=abs(hdp)
    raw_ev = 0.0
    if h==0: raw_ev = (w2+w1)*b-(l1+l2)
    elif fav:
        if h==0.25: raw_ev = (w2+w1)*b-d*0.5-(l1+l2)
        elif h==0.5: raw_ev = (w2+w1)*b-(d+l1+l2)
        elif h==0.75: raw_ev = w2*b+w1*(b/2)-(d+l1+l2)
        elif h==1.0: raw_ev = w2*b-(d+l1+l2)
        elif h==1.25: raw_ev = w2*b-w1*0.5-(d+l1+l2)
        elif h==1.5: raw_ev = w2*b-(w1+d+l1+l2)
    else:
        if h==0.25: raw_ev = (w2+w1)*b+d*(b/2)-(l1+l2)
        elif h==0.5: raw_ev = (w2+w1+d)*b-(l1+l2)
        elif h==0.75: raw_ev = (w2+w1+d)*b-l1*0.5-l2
        elif h==1.0: raw_ev = (w2+w1+d)*b-l2
        elif h==1.25: raw_ev = (w2+w1+d)*b+l1*(b/2)-l2
        elif h==1.5: raw_ev = (w2+w1+d+l1)*b-l2
    return apply_complexity_penalty(raw_ev, h)
...
def ev_ou(line,pt,odds,over):
    ...
    raw_ev = 0.0
    if over:
        if rm==0.0:  raw_ev = g(lambda k:k>fl)*b - g(lambda k:k<fl)
        elif rm==0.25: raw_ev = g(lambda k:k>=fl+1)*b - pt.get(fl,0)*0.5 - g(lambda k:k<fl)
        elif rm==0.5:  raw_ev = g(lambda k:k>=fl+1)*b - g(lambda k:k<=fl)
        elif rm==0.75: raw_ev = g(lambda k:k>=fl+2)*b + pt.get(fl+1,0)*(b/2) - g(lambda k:k<=fl)
    else:
        if rm==0.0:  raw_ev = g(lambda k:k<fl)*b - g(lambda k:k>fl)
        elif rm==0.25: raw_ev = g(lambda k:k<fl)*b + pt.get(fl,0)*(b/2) - g(lambda k:k>=fl+1)
        elif rm==0.5:  raw_ev = g(lambda k:k<=fl)*b - g(lambda k:k>=fl+1)
        elif rm==0.75: raw_ev = g(lambda k:k<=fl)*b - pt.get(fl+1,0)*0.5 - g(lambda k:k>=fl+2)
    return apply_complexity_penalty(raw_ev, line)
...
<div style="display:flex;align-items:center;gap:14px;margin-bottom:4px;">
  ...
  <div style="text-align:right;">
    ...
    <span class="gem-badge">● QUANT ONLINE</span>
  </div>
</div>
...
{"match_name":"","h1x2_val":0.0,"d1x2_val":0.0,"a1x2_val":0.0,"hdp_line_val":0.0,"hdp_h_w_val":0.0,"hdp_a_w_val":0.0,"ou_line_val":0.0,"ou_over_w_val":0.0,"ou_under_w_val":0.0}"""
    ...
    if st.button("⚡  RUN ORACLE ANALYSIS",use_container_width=True,type="primary"):
        ...
        # 🌟 3. ระบบกระจายความเสี่ยง (Cross-Market Dutching)
        valid_bets = []
        ...
        if valid_bets:
            if not api_key: st.warning("API Key required for Oracle")
                ...
                with st.spinner("◈ THE ORACLE PROCESSING (DUTCHING MODE)..."):
                    for tc in valid_bets:
                        tf=None
                        if tc['n']=="เจ้าบ้าน": tf=fav_h
                        elif tc['n']=="ทีมเยือน": tf=not fav_h
                        
                        # 🌟 2. ลดความเสี่ยงจาก Extreme Odds (FLB)
                        flb_penalty = 0.0
                        if tc['odds'] < 1.30 or tc['odds'] > 3.00: flb_penalty = 0.03 # โดนทำโทษลด EV 3% หากอยู่ในโซน Extreme Odds
                        ...
                        if v.get('final_decision',False) and nev>0:
                            ...
                            # ถ้าเป็น Dutching Mode (เข้า 2 บิล) ให้แบ่งไม้ลงทุนละครึ่ง (0.5) ของที่คำนวณได้
                            dutch_factor = 0.5 if len(valid_bets) == 2 else 1.0
...
{"match_name":"","current_min":0,"current_score_h":0,"current_score_a":0,"rc_h":false,"rc_a":false,"live_hdp":0.0,"live_hdp_h":0.0,"live_hdp_a":0.0,"live_ou":0.0,"live_ou_over":0.0,"live_ou_under":0.0}'''
    ...
    if snap:
        ...
        # 🌟 3. ระบบกระจายความเสี่ยงสำหรับบอลสด (Live Cross-Market Dutching)
        valid_bets_live = []
            ...
            if not api_key: st.warning("API Key required")
                ...
                with st.spinner("◈ SNIPER ORACLE PROCESSING..."):
                    for tl2 in valid_bets_live:
                        ...
                        # 🌟 2. ลดความเสี่ยงจาก Extreme Odds
                        flb_penalty = 0.0
                        if tl2['odds'] < 1.30 or tl2['odds'] > 3.00: flb_penalty = 0.03
                        ...
                        if al.get('final_decision',False) and nlev>=lim:
                            ...
                            dutch_factor = 0.5 if len(valid_bets_live) == 2 else 1.0
