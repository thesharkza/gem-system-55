            ...
            if st.button("🧠 สั่ง AI วิเคราะห์ความผิดพลาด (Post-Mortem)", use_container_width=True):
                with st.spinner("The Oracle กำลังสแกนหา 'กับดักราคา' จากประวัติความพ่ายแพ้..."):
                    ...
                    prompt_debrief = (
                        ...
                        "สร้างกฎเหล็กข้อใหม่ 1-3 ข้อ ที่ 'เจาะจง' 'วัดผลได้' และ 'อุดช่องโหว่ได้ 100%' เพื่อป้องกันไม่ให้ระบบเสียเงินกับรูปแบบเดิมอีก โดยเขียนตาม Format นี้:\n\n"
                        "Gem : [หมวดหมู่] (ชื่อกฎ) | เงื่อนไขที่ชัดเจน (เช่น นาที, เรตราคา) -> การตัดสินใจ (เช่น สั่งทับมือ, ลดไม้)\n"
                        "*ตัวอย่าง: Gem : [Live O/U Trap] (กับดักสกอร์สูงท้ายเกม) หากเป็นบอลสดนาที 75+ สกอร์ขาดไปแล้ว 2 ลูก แต่เรตสูงต่ำยังเปิดวัดใจที่ 0.5/1 ค่าน้ำ < 0.80 -> ถือเป็นกับดักล่อเม่า สั่งทับมือเด็ดขาด*\n"
                    )
