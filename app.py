...
# 🆕 เพิ่มสัญญาณที่ 5 (236 เคส): ah_home_odds≤1.90 (ตลาดเชียร์เจ้าบ้าน)
#   เดี่ยว WR 60% (n=108, p=0.021) · odds≤1.90 vs >1.90 = 68% vs 49% (แยกชัด)
#   5-signal scoring: เล่นเมื่อ≥4 → WR 62% (n=68, p=0.034) ดีกว่าเกณฑ์เดิม ≥3→57%
#   ⚠️ 5-fold ±12% ผันผวนกว่า "O/U สูง" — เป็นสัญญาณเสริม ไม่ใช่ตัวหลัก
# 5 สัญญาณ: ค่าน้ำ O/U สูง, stat เชียร์ home, home ฟอร์มดี, home λ สูง, ตลาดเชียร์(odds≤1.90)
# ════════════════════════════════════════════════════════════════════════
def ah_home_confidence(p):
    """นับสัญญาณสนับสนุน AH Home (0-5) — ยิ่งสูงยิ่งน่าเล่น AH Home
    คืน (score, list ของสัญญาณที่เข้า)
    สัญญาณที่ 5 (odds≤1.90) เพิ่มจากวิเคราะห์ 236 เคส: ทำให้ scoring แม่นขึ้น
    (5 สัญญาณ→75%, เล่นเมื่อ≥4→62% ดีกว่าเกณฑ์เดิม ≥3→57%)"""
    def _nz(v, d=0):
        if v is None: return d
        try:
            if pd.isna(v): return d
    ...
    if 0 < _nz(p.get('ah_home_odds')) <= 1.90:
        hits.append('ตลาดเชียร์เจ้าบ้าน (ราคา≤1.90)')
...
def compute_edge_signals(p):
    ...
    if ah_score >= 4:
        signals.append({
            'label': f'🏠 AH HOME มั่นใจสูง ({ah_score}/5 สัญญาณ)',
            'status': 'strong',
            'detail': f'≥4 สัญญาณหนุนเจ้าบ้าน (อดีต WR ~62-75%) → เล่น AH Home น่าสนใจ'
        })
    elif ah_score == 3:
        signals.append({
            'label': f'🏠 AH Home ค่อนข้างมั่นใจ ({ah_score}/5)',
            'status': 'positive',
            'detail': 'สัญญาณหนุนเจ้าบ้าน 3/5 (อดีต WR ~52-57%)'
        })
...
with tab_pre:
    ...
    if not valid_odds:
        ...
        if valid_sides:
            ...
            # หลักฐาน: AH Home conf≥4 (จาก 5 สัญญาณ) → WR ~62% (เพิ่ม odds≤1.90 เป็นสัญญาณที่ 5)
            ah_conf_score = 0
            ...
            ah_home_override = (best['name'] == 'AH Home' and ah_conf_score >= 4)
            ...
            if ah_home_override and risky_zone:
                st.markdown(
                    ...
                    f'แม้อยู่โซนเสี่ยง แต่ AH Home มีสัญญาณหนุนเจ้าบ้าน {ah_conf_score}/5 '
                    f'(home advantage — อดีต WR ~62%, เพิ่มสัญญาณราคาตลาด) '
...
with tab_backtest:
    ...
    if len(settled) < 3:
        ...
        edge_defs = [
            ('🏠 AH HOME ≥4/5 สัญญาณ → Home', lambda p: ah_home_confidence(p)[0] >= 4, _home_outcome),
